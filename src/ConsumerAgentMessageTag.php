<?php
/**
 * ConsumerAgentMessageTag automatically generated by SDKgen please do not edit this file manually
 * @see https://sdkgen.app
 */

namespace Fusio\Sdk;

use GuzzleHttp\Exception\BadResponseException;
use Sdkgen\Client\Exception\ClientException;
use Sdkgen\Client\Exception\Payload;
use Sdkgen\Client\Exception\UnknownStatusCodeException;
use Sdkgen\Client\TagAbstract;

class ConsumerAgentMessageTag extends TagAbstract
{
    /**
     * Returns a paginated list of agent messages
     *
     * @param string $agentId
     * @param int|null $refId
     * @param string|null $chatId
     * @return ConsumerAgentMessageCollection
     * @throws CommonMessageException
     * @throws ClientException
     */
    public function getAll(string $agentId, ?int $refId = null, ?string $chatId = null): ConsumerAgentMessageCollection
    {
        $url = $this->parser->url('/consumer/agent/$agent_id<[0-9]+|^~>/message', [
            'agent_id' => $agentId,
        ]);

        $options = [
            'headers' => [
            ],
            'query' => $this->parser->query([
                'ref_id' => $refId,
                'chat_id' => $chatId,
            ], [
            ]),
        ];

        try {
            $response = $this->httpClient->request('GET', $url, $options);
            $body = $response->getBody();

            $data = $this->parser->parse((string) $body, \PSX\Schema\SchemaSource::fromClass(ConsumerAgentMessageCollection::class));

            return $data;
        } catch (ClientException $e) {
            throw $e;
        } catch (BadResponseException $e) {
            $body = $e->getResponse()->getBody();
            $statusCode = $e->getResponse()->getStatusCode();

            if ($statusCode >= 0 && $statusCode <= 999) {
                $data = $this->parser->parse((string) $body, \PSX\Schema\SchemaSource::fromClass(CommonMessage::class));

                throw new CommonMessageException($data);
            }

            throw new UnknownStatusCodeException('The server returned an unknown status code: ' . $statusCode);
        } catch (\Throwable $e) {
            throw new ClientException('An unknown error occurred: ' . $e->getMessage());
        }
    }

    /**
     * Submits a new agent message
     *
     * @param string $agentId
     * @param AgentInput $payload
     * @param int|null $refId
     * @return AgentOutput
     * @throws CommonMessageException
     * @throws ClientException
     */
    public function submit(string $agentId, AgentInput $payload, ?int $refId = null): AgentOutput
    {
        $url = $this->parser->url('/consumer/agent/$agent_id<[0-9]+|^~>/message', [
            'agent_id' => $agentId,
        ]);

        $options = [
            'headers' => [
                'Content-Type' => 'application/json',
            ],
            'query' => $this->parser->query([
                'ref_id' => $refId,
            ], [
            ]),
            'json' => $payload,
        ];

        try {
            $response = $this->httpClient->request('POST', $url, $options);
            $body = $response->getBody();

            $data = $this->parser->parse((string) $body, \PSX\Schema\SchemaSource::fromClass(AgentOutput::class));

            return $data;
        } catch (ClientException $e) {
            throw $e;
        } catch (BadResponseException $e) {
            $body = $e->getResponse()->getBody();
            $statusCode = $e->getResponse()->getStatusCode();

            if ($statusCode >= 0 && $statusCode <= 999) {
                $data = $this->parser->parse((string) $body, \PSX\Schema\SchemaSource::fromClass(CommonMessage::class));

                throw new CommonMessageException($data);
            }

            throw new UnknownStatusCodeException('The server returned an unknown status code: ' . $statusCode);
        } catch (\Throwable $e) {
            throw new ClientException('An unknown error occurred: ' . $e->getMessage());
        }
    }



}
